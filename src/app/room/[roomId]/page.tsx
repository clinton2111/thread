...
const Page = () => {
  ...
  const timeRemaining =
    ttlData?.ttl === undefined ? null : Math.max(0, ttlData.ttl - Math.floor((now - ttlUpdatedAt) / 1000));
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
        ...
        {messages?.messages.map((msg) => (
          <div key={msg.id} className={`flex flex-col ${msg.sender === username ? "items-start" : "items-end"}`}>
            <div
              className={`max-w-[80%] group ${msg.sender === username ? "text-left" : "text-right"} rounded-lg border px-3 py-2 bg-zinc-900/60 border-zinc-800`}
            >
            ...
            <span className={`text-[10px] text-zinc-600 mt-1 ${msg.sender === username ? "self-start" : "self-end"}`}>
